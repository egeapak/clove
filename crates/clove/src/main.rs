...
fn run_repo(
    ...
    match command {
        ...
        Commands::Ready(a) => cmd::ready::run(ctx, f, a, quiet).map(|_| ok),
        Commands::Blocked(a) => cmd::blocked::run(ctx, f, a, quiet).map(|_| ok),
        Commands::Ls(a) => cmd::ls::run(ctx, f, a).map(|_| ok),
        Commands::Query(a) => cmd::query::run(ctx, f, a).map(|_| ok),
        ...
        Commands::Doctor(a) => cmd::doctor::run(ctx, f, a),
